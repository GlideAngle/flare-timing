...
module Shake.General.Timing(resetTimings, addTiming, printTimings) where
...
showTimings stop times = showGap $
    ...
    where
        ...
        showPerc x = let s = show ((floor $ x * 100 // sm) :: Int) in replicate (3 - length s) ' ' ++ s ++ "%"
        progress x = let i = ((floor $ x * 25 // mx) :: Int) in replicate i '=' ++ replicate (25-i) ' '
