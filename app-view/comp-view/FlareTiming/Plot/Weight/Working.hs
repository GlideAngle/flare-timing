...
viewWeightWorking
    ...
    -> m ()
viewWeightWorking hgOrPg vy' tw' al' = do
    _ <- dyn $ ffor3 vy' tw' al' (\vy tw al ->
        case (vy, tw, al) of
            (Nothing, _, _) -> text "Loading validity ..."
            (_, Nothing, _) -> text "Loading competition tweaks ..."
            (_, _, Nothing) -> text "Loading allocations ..."
            (Just Vy.Validity{task}, Just tweak, Just alloc) -> do
