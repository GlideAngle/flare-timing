...
timeUnits = testGroup "Time tests"
    [ HU.testCase "1 hr = 1:00:00" $ (1 % 1) `compare` (hms 1 0 0) @?= EQ 
    , HU.testCase "2 hr = 2:00:00" $ (2 % 1) `compare` (hms 2 0 0) @?= EQ 
    , HU.testCase "3 hr = 3:00:00" $ (3 % 1) `compare` (hms 3 0 0) @?= EQ 
    ]

maxUnits :: TestTree
maxUnits = testGroup "Maximum tests"
    [ HU.testCase "1 hr best time, 1:00:00 pilot time = 1 speed fraction" $
        FS.speedFraction (BestTime (1 % 1)) (PilotTime $ hms 1 0 0) `compare` maxS
        @?= EQ 
     
    , HU.testCase "2 hr best time, 2:00:00 pilot time = 1 speed fraction" $
        FS.speedFraction (BestTime (2 % 1)) (PilotTime $ hms 2 0 0) `compare` maxS
        @?= EQ 
     
    , HU.testCase "3 hr best time, 3:00:00 pilot time = 1 speed fraction" $
        FS.speedFraction (BestTime (3 % 1)) (PilotTime $ hms 3 0 0) `compare` maxS
        @?= EQ 

    , HU.testCase "1 hr best time, 1:00:01 pilot time < 1 speed fraction" $
        FS.speedFraction (BestTime (1 % 1)) (PilotTime $ hms 1 0 1) `compare` maxS
        @?= LT
     
    , HU.testCase "2 hr best time, 2:00:01 pilot time < 1 speed fraction" $
        FS.speedFraction (BestTime (2 % 1)) (PilotTime $ hms 2 0 1) `compare` maxS
        @?= LT
     
    , HU.testCase "3 hr best time, 3:00:01 pilot time < 1 speed fraction" $
        FS.speedFraction (BestTime (3 % 1)) (PilotTime $ hms 3 0 1) `compare` maxS
        @?= LT
    ]
...
minUnits = testGroup "Minimum tests"
    [ HU.testCase "1 hr best time, 1:59:59 pilot time > 0 speed fraction" $
        FS.speedFraction (BestTime (1 % 1)) (PilotTime $ hms 1 59 59) `compare` minS
        @?= GT
     
    , HU.testCase "1 hr best time, 2:00:00 pilot time = 0 speed fraction" $
        FS.speedFraction (BestTime (1 % 1)) (PilotTime $ hms 2 0 0) `compare` minS
        @?= EQ
